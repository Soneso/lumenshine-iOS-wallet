...
class WalletDetailsViewController: UIViewController {
    ...
    private func setupBalances() {
        ...
        for currency in wallet.balances {
            if let assetCode = currency.assetCode, let balance = CoinUnit(currency.balance), let available = CoinUnit(currency.balance)?.availableAmount(forWallet: wallet, forCurrency: currency){
                if wallet.isCurrencyDuplicate(withAssetCode: assetCode), let issuer = currency.assetIssuer {
                    ...
                    availableValues.append("\(available) \(assetCode) (\(issuer.prefix(4))...)\n")
                } else {
                    balanceValues.append("\(balance) \(assetCode)\n")
                    availableValues.append("\(available) \(assetCode)\n")
                }
            }
        }
